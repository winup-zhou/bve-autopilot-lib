...
namespace autopilot
{
	...
	void ato::信号制限区間追加(
        m 直前位置, double 距離, int 制限, const 共通状態& 状態)
	{
		制限区間追加(
			_制限速度, 制限+距離*1000, { 直前位置, 状態.現在位置() });
	}
    ...
    void ato::経過(const 共通状態 &状態, const tasc &tasc状態)
    {
        ...
        if (_制御状態 == 制御状態::停止) {
            ...
            /*auto 制動成分 = _出力ノッチ.制動成分();
            if (状態.現在時刻() < _発進時刻 + 状態.設定().加速終了遅延()) {
                制動成分 = 自動制動自然数ノッチ{ static_cast<unsigned int>(std::ceil(状態.制動().自動最大ノッチ().value * 0.25)) };
            }*/
        }
